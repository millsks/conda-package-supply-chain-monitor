...
How a component built from this platform is declared, released and shut down:
the two declarations, the process model, the release stage and the probes.

What Conda-Sentinel's own collectors and policies need in a deployment — every
one of which ships inert until it is told where to look — is
[operating Conda-Sentinel](../conda-sentinel/operations.md).
